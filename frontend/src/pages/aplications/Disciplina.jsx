...
const Disciplina = () => {
  ...
  const ROLES_PERMITIDOS = ['Administrador', 'Instructor', 'Cadete'];
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Cadete solo necesita acciones definidas (para mi perfil)
  ...
  const esCadete = usuarioActual?.rol === 'Cadete';
      ...
      <div className={`flex-1 flex flex-col overflow-hidden transition-all duration-300 ${
            ...
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center group">
                <div className="group-hover:scale-105 transition-transform duration-300">
                  <h1 className="text-2xl lg:text-3xl font-black bg-linear-to-r from-slate-800 via-slate-700 to-slate-900 bg-clip-text text-transparent leading-tight">
                    CONTROL DISCIPLINARIO
                  </h1>
                  <p className="text-sm lg:text-base text-slate-500 font-medium tracking-wide -mt-1">
                    {esCadete ? 'Mi Perfil Disciplinario' : 'Registro de Acciones Positivas y Negativas'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                {usuarioActual && (
                  <div className="text-right hidden md:flex flex-col items-end gap-0.5 pr-4 border-r border-slate-200">
                    <p className="font-bold text-slate-900 text-sm lg:text-base leading-tight">
                      {usuarioActual.gradoU}. {usuarioActual.nombreU}
                    </p>
                ...
                <div className="relative group">
                  <div className="w-14 h-14 bg-linear-to-br from-blue-500 via-blue-600 to-indigo-600 hover:from-blue-600 hover:via-blue-700 hover:to-indigo-700 rounded-2xl shadow-xl flex items-center justify-center transition-all duration-300 group-hover:scale-110 cursor-pointer border-4 border-white/50">
        ...
        <main className="flex-1 overflow-y-auto p-6 pt-4 pb-20">
          {activeSection === 'miPerfil' && (
            <MiPerfilSection usuarioActual={usuarioActual} />
          )}

          {activeSection === 'dashboard' && !esCadete && (
            <DashboardSection
              cadetes={cadetes}
              acciones={acciones}
          ...
          {activeSection === 'cadetes' && !esCadete && (
            <CadetesSection
              cadetes={cadetes}
              acciones={acciones}
          ...
          {activeSection === 'registrar' && !esCadete && (
            <RegistrarSection
              cadetes={cadetes}
          ...
          {activeSection === 'historial' && !esCadete && (
            <HistorialSection acciones={acciones} cadetes={cadetes} />
          )}

          {activeSection === 'estadisticas' && !esCadete && (
            <EstadisticasSection cadetes={cadetes} acciones={acciones} />
          )}

          {activeSection === 'configuracion' && usuarioActual?.rol === 'Administrador' && (
